...
export type LocalPlan = 'pdf' | 'basic' | 'workbook' | 'igent7' | 'igent30' | 'igent90' | 'group' | 'vip';
...
export function localEntitlements(plan: LocalPlan): ProductKey[] {
  const matrix: Record<LocalPlan, ProductKey[]> = {
    ...
    igent7: [PRODUCT_KEYS.pdf, PRODUCT_KEYS.base, PRODUCT_KEYS.workbook, PRODUCT_KEYS.igentMind7],
...
export function hasLocalEntitlement(plan: LocalPlan, productKey: ProductKey) {
  if (productKey === PRODUCT_KEYS.igentMind7 || productKey === PRODUCT_KEYS.igentMind30 || productKey === PRODUCT_KEYS.igentMind90) {
    const entitlements = localEntitlements(plan);
    return entitlements.includes(PRODUCT_KEYS.igentMind7) || entitlements.includes(PRODUCT_KEYS.igentMind30) || entitlements.includes(PRODUCT_KEYS.igentMind90);
  }
